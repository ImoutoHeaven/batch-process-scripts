...
DEPRECATION_NOTICE = """DEPRECATED: adv7z.py is frozen and not maintained.
Supported entrypoint: python3 advArchiver/advArchiver.py 7z ...
...
def check_required_tools(no_par2=False):
    ...
    if missing_tools:
        ...
        if "parpar" in missing_tools:
            ...
            error_msg += (
                "  提示: 如果不需要PAR2恢复记录，可以使用 --no-rec 参数跳过此检查\n"
            )
    ...
    if platform.system() == "Windows":
        ...
        if os.path.basename(short_dir).startswith("-") and not wildcard_part:
            short_dir = os.path.join(
                os.path.dirname(short_dir), "." + os.sep + os.path.basename(short_dir)
            )
        ...
        if os.path.basename(dir_part).startswith("-") and not wildcard_part:
            unix_path = (
                os.path.join(
                    os.path.dirname(dir_part), "./" + os.path.basename(dir_part)
                )
                + wildcard_part
            )
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="7z压缩工具（含PAR2恢复记录，多线程支持）",
    ...
    # 并发线程数（新增）
    parser.add_argument(
        "-t", "--threads", type=int, default=1, help="同时进行压缩的任务数 (默认: 1)"
    )
    ...
    parser.add_argument(
        "--no-emb", action="store_true", help="生成独立的PAR2文件，不嵌入到7z文件中"
    )
    parser.add_argument(
        "--no-rec", action="store_true", help="不生成PAR2恢复记录（跳过parpar工具检查）"
    )
...
def find_and_rename_7z_files(
    ...
    if not valid_part_files:
        print(
            f"错误: 没有找到7z文件 {temp_name_prefix}.7z 或分卷文件 {temp_name_prefix}.7z.xxx"
        )
        return False, []
...
def process_folder(folder_path, args, base_path):
    ...
    final_target_dir = (
        safe_abspath(args.out) if args.out else os.path.dirname(folder_path)
    )
    safe_makedirs(final_target_dir, exist_ok=True, debug=args.debug)
...
def main():
    ...
    if is_parted_profile(args.profile):
        if not args.no_rec:
            ...
            if not args.no_emb:
                ...
                print(
                    "注意: 分卷模式自动使用独立PAR2文件（不嵌入），以确保分卷文件格式兼容性"
                )
    ...
    if args.ext_skip_folder_tree:
        if not args.skip_extensions:
            error_msg = (
                "错误: --ext-skip-folder-tree 参数只有在指定 --skip-${ext} 参数时才有效"
            )
            stats.log(error_msg)
            print(error_msg)
            sys.exit(1)
