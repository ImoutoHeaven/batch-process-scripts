...
DEPRECATION_NOTICE = """DEPRECATED: advRar.py is frozen and not maintained.
Supported entrypoint: python3 advArchiver/advArchiver.py rar ...
...
def move_files_to_final_destination(
    temp_dir, final_output_dir, moved_files, debug=False
):
    """
    将文件从临时目录移动到最终目的地
...
def parse_arguments():
    ...
    # 并发线程数（新增）
    parser.add_argument(
        "-t", "--threads", type=int, default=1, help="同时进行压缩的任务数 (>=1)"
    )
...
def find_and_rename_rar_files(
    ...
    if not part_files:
        print(
            f"错误: 没有找到RAR文件 {temp_name_prefix}.rar 或分卷文件 {temp_name_prefix}.part*.rar"
        )
        return False, []
...
def process_file(file_path, args, base_path):
    ...
    try:
        # 构建 RAR 命令
        rar_switches = build_rar_switches(
            args.profile, args.password, args.delete, args.comment_switch
        )
        ...
        if result.returncode == 0:
            ...
            ok_rename, moved = find_and_rename_rar_files(
                "temp_archive", final_name, temp_dir, args.debug
            )
            if ok_rename:
                ok_move, finals = move_files_to_final_destination(
                    temp_dir, final_output_dir, moved, args.debug
                )
                if ok_move:
                    stats.add_success("文件", abs_file_path)
                else:
                    stats.add_failure(
                        "文件", abs_file_path, 0, "移动RAR到最终目录失败", cmd_str
                    )
            else:
        ...
        if result.returncode == 0:
            ...
            if ok_rename:
                ...
                if ok_move:
                    ...
                    stats.add_failure(
                        "文件夹", folder_path, 0, "移动RAR到最终目录失败", cmd_str
                    )
            else:
